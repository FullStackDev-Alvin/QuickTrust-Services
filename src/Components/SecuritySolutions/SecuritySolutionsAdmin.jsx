...
function SecuritySolutionsAdmin() {
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      formData.append("key_features", editItem.key_features);
      formData.append("short_description", editItem.short_description);
  ...
  return (
    <div className="w-[94%] m-auto p-5 flex flex-wrap gap-[20px] justify-center h-screen overflow-scroll">
      {data.map((item) => (
  <div key={item.id || item._id} className="w-[300px] max-h-[600px] h-auto bg-text p-2 flex flex-col gap-2">
    {/* Display the first image from the image array */}
    <img
      src={item.image ? JSON.parse(item.image)[0] : "default-image.jpg"}  // Parse image URL string into array
    ...
    <div className="w-full flex flex-col gap-[10px] justify-center items-center">
      ...
      {showModal && selectedItem && (
        <div className="w-full h-screen fixed flex justify-center items-center inset-0 bg-black bg-opacity-50 p-4">
          ...
          {/* Close Button */}
          <button 
            className="bg-button p-2 text-sm md:text-base font-bold fixed right-[5%] top-[10%] text-primary rounded-md" 
...
export default SecuritySolutionsAdmin;
