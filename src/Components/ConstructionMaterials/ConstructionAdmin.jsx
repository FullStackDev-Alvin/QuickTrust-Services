...
function ConstructionAdmin() {
  ...
  const handleEditSubmit = async () => {
    try {
      ...
      await axios.put(`https://quicktrustservices-i6rr.vercel.app/api/construction/${id}`, formData, {
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(`https://quicktrustservices-i6rr.vercel.app/api/construction/${id}`);
  ...
  return (
    <div className="w-[94%] m-auto p-5 flex flex-wrap gap-[20px] justify-center">
      {data.map((item) => (
        <div key={item.id || item._id} className="w-[300px] max-h-[450px] h-auto bg-text p-2 flex flex-col gap-2">
          <img
...
export default ConstructionAdmin;
